...
out_pert_ref = rep_stoch_pert_sim (deb=0,h=Times_obs[length(Times_obs)]/5000,fin=Times_obs[length(Times_obs)],Pars = theta,y0= x0,sd_stoch_pert = sig_c)
out_pert = t(out_pert_ref[[2]][,seq(1,length(out_pert_ref[[1]]),5000/25)])
Obs_Y =  matrix(data = rep(0, nb_obs*dim_obs), nrow = nb_obs, ncol = dim_obs )
for (col_i in 1:dim_obs) Obs_Y[,col_i] = pmax(out_pert[,col_i]+sigma_obs*rnorm(n = nb_obs),matrix(0.1,nb_obs,1))
Obs_Y = t(Obs_Y)
...
lambda_trial = c(10,20,50,100,200)
...
for (lambda  in lambda_trial){
  ...
  res_est = est_param_iter_oca_nonlinear_profiled_ic(Times_obs,Obs_Y,State_ini,
                                                     param_ini=theta_ini_prof_ini,mat_U,mat_A=Repressilator_matA,mat_B,mat_C,mesh_mult=20,x0_known =  c(x0_pro,1),type_opt =0)

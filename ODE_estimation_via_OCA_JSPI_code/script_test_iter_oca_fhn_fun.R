...
fhn_sim = function(Time, State, Pars){fitz_nagumo_fun_sim(Time, State, Pars,fun_a)}
out <- ode(func = fhn_sim, y = x0 , parms =theta, times = Times_obs)

Times_obs <- out[,"time"]
Times_obs_ext = c(Times_obs,21)
X_without_noise = out[,2:3];

Obs_Y =  matrix(data = rep(0, nb_obs*dim_obs), nrow = nb_obs, ncol = dim_obs )
for (col_i in 1:dim_obs) Obs_Y[,col_i] = X_without_noise[,col_i]+sigma_obs*rnorm(n = nb_obs)
Obs_Y = t(Obs_Y)

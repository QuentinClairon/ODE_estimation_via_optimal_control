...
out1 <- ode(func = Microbiota_sim, y = x0_int_mic1 , parms =theta, times = Times_obs)
out2 <- ode(func = Microbiota_sim, y = x0_int_mic2 , parms =theta, times = Times_obs)
out3 <- ode(func = Microbiota_sim, y = x0_int_mic3 , parms =theta, times = Times_obs)

mean_state_magn1 = colSums(abs(log(out1[,ind_state_sel+1])))/length(Times_obs)
mean_state_magn2 = colSums(abs(log(out2[,ind_state_sel+1])))/length(Times_obs)
mean_state_magn3 = colSums(abs(log(out3[,ind_state_sel+1])))/length(Times_obs)

Time_list = append(list(Times_obs),list(Times_obs))
Time_list = append(Time_list,list(Times_obs))

Obs_Y1 = log(out1[,ind_state_sel+1]) + sigma_obs*matrix(rnorm(n = nb_obs*dim_syst),nb_obs,dim_syst)*matrix(mean_state_magn1,nb_obs,dim_syst)
Obs_Y2 = log(out2[,ind_state_sel+1]) + sigma_obs*matrix(rnorm(n = nb_obs*dim_syst),nb_obs,dim_syst)*matrix(mean_state_magn2,nb_obs,dim_syst)
Obs_Y3 = log(out3[,ind_state_sel+1]) + sigma_obs*matrix(rnorm(n = nb_obs*dim_syst),nb_obs,dim_syst)*matrix(mean_state_magn3,nb_obs,dim_syst)

est_param_iter_oca_nonlinear_profiled_ic <- function(Times_obs,Yn,State_ini,param_ini,mat_U,mat_A,mat_B,mat_C,mesh_mult=1,x0_known = c(),type_opt =1){
  ############################################################
  # DESCRIPTION: compute the parameter estimation via the method presented in article when profiling on ic and for nonlinear ODE models
  # 
  # FUNCTION SIGNATURE:  est_param_iter_oca_nonlinear_profiled_ic <- function(Times_obs,Yn,State_ini,param_ini,mat_U,mat_A,mat_B,mat_C,mesh_mult=1,x0_known = c(),type_opt =1)
  # 
  # INPUT :
  ...
  func_eco_E <- function(param){
    ...
    ensemble_Ricatti_comp =  compute_Riccati_nonlinear_profiled_ic(Times_integ,pseudo_Y,State_i_m1 ,param,mat_U,mat_A,mat_B,mat_C,weight_integ,x0_known)
  ...
  return(list(est_param = est_param,set_Ricatti_State_opt_value = ensemble_Ricatti_final, Times_integ= Times_integ,err_pred = err_pred))
}